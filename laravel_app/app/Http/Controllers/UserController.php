...
class UserController extends Controller
{
    ...
    public function createItem($item_name, $item_effect, $type_object, $id_player, $nbr) {
    ...
    public function createSpell($spell_name, $spell_effect, $ecole, $id_player) {
        ...
        $sqlPerso = DB::table('spells')->insertGetId(
            ['id_player' => $id_player, 'spellName' => $spell_name, 'spellEffect' => $spell_effect, 'école' => $ecole, 'cost' => $cost]
        );

        return json_encode("Insert Done !");
    }

    public function updatePosition($listPosition) {
    ...
    public function updateStats($health, $maxHealth, $mana, $maxMana, $id) {
        $user = DB::table('stats')
        ->where('id_player', $id)
    ...
    public function addToGroup($id, $active) {
        if($active == 1) {
            $active++;
        }else {
            $active--;
    ...
    public function createCharacter($name, $special, $race) {
